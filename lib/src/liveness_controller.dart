...
class LivenessController extends ChangeNotifier {
  ...
  /// Non-null (0.0–1.0) while the YOLOv8n-face model is being downloaded for
  ...
  Future<void> initialize() async {
    try {
      ...
      if (_config.enableVideoReplayDetection) {
        try {
          ...
          _videoReplay = TFLiteService(
            ...
            cropScale:        isCustomModel ? 1.4 : VideoReplayModelDownloader.bundledCropScale,
            useImageNetBgr:   isCustomModel ? false : VideoReplayModelDownloader.bundledUseImageNetBgr,
          );
          ...
          if (!loaded) {
            ...
            debugPrint('[LivenessController] VideoReplay model FAILED to load — path=$modelPath');
            _videoReplay = null;
          } else {
            debugPrint('[LivenessController] VideoReplay model loaded OK — path=$modelPath inputSize=$inputSize realClassIndex=${VideoReplayModelDownloader.bundledRealClassIndex}');
          }
        } catch (e) {
      ...
      // YOLOv8n-face — opt-in alternative detector for the identity pipeline
      // only (see FaceDetectorBackend doc comment). Same isolation pattern as
      // FaceNet above: a failed download/load disables the yolov8 backend for
      // this session and identity silently falls back to ML Kit's own
      // landmarks rather than blocking the session.
      if (_config.enableFaceId && _config.faceDetectorBackend == FaceDetectorBackend.yolov8) {
        try {
          _yoloDetector = YoloFaceDetectorService();
          await _yoloDetector!.load(
            modelUrl: _config.yoloModelUrl,
            onProgress: (p) {
              _yoloModelDownloadProgress = p;
              notifyListeners();
            },
          );
          if (_isDisposed) return;
          ...
          debugPrint('[LivenessController] YOLOv8 face detector unavailable this session: $e');
          _yoloDetector = null;
          _yoloModelDownloadProgress = null;
        }
      }
  ...
  Future<void> _processFrame(
    ...
    if ((_config.enableFaceId || _config.enableBestFrontalCapture) &&
      ...
      if (_config.enableFaceId &&
        ...
        final detections = await _yoloDetector!.detectFromRawFrame(
          imageBytes: result.rawFrame!.imageBytes,
          imageWidth: result.rawFrame!.imageWidth,
          imageHeight: result.rawFrame!.imageHeight,
      ...
      final hasEyesForBackend = _config.faceDetectorBackend == FaceDetectorBackend.yolov8
          ...
          : (_currentFace!.leftEyePosition != null && _currentFace!.rightEyePosition != null);
      if (isCandidateForCollection && hasEyesForBackend) {
        _frontalFrames.add((raw: result.rawFrame!, face: _currentFace!, yolo: yoloFace));
      }
    }
    ...
    if (_config.enableVideoReplayDetection &&
      ...
      if (_faceCropBrightnessHistory.length > 60) _faceCropBrightnessHistory.removeAt(0);
      if (_faceCropBrightnessHistory.length >= 10) {
        final v = _faceCropBrightnessVariance();
        _liveHeuristicScore = (v / 0.0005).clamp(0.0, 1.0);
        debugPrint('[VR-BRIGHT] brightVar=${v.toStringAsFixed(7)} score=${_liveHeuristicScore!.toStringAsFixed(3)}');
      }
      ...
      final lap = _faceCropLaplacianVariance(rawForTflite, faceForTflite.boundingBox);
      _faceCropLaplacianHistory.add(lap);
      if (_faceCropLaplacianHistory.length > 60) _faceCropLaplacianHistory.removeAt(0);
      if (_faceCropLaplacianHistory.isNotEmpty) {
        _liveLaplacianScore = _faceCropLaplacianHistory.reduce((a, b) => a + b) /
  ...
  Future<void> _onEngineComplete(LivenessResult result) async {
    ...
    if (_config.enableVideoReplayDetection) {
      ...
      final meanLap = _faceCropLaplacianHistory.isNotEmpty
          ? _faceCropLaplacianHistory.reduce((a, b) => a + b) / _faceCropLaplacianHistory.length
          : null;
      ...
      final modelScore = (_lastVideoReplayScore != null && _lastVideoReplayScore! < 0.85)
          ? _lastVideoReplayScore!
          : null;

      // S5 — ReplayAnalyzer (perceptual fingerprint + micro-jitter + entropy + blink)
      final raResult = _replayAnalyzer.buildResult(threshold: _config.videoReplayThreshold);
      ...
      finalResult = finalResult.withVideoReplayResult(finalScore, videoReplayDetected: isReplay);
    }
    ...
    if (finalResult.videoReplayDetected) {
      final scoreStr = finalResult.videoReplayScore != null
          ...
          : '';
      onFailed('Video replay attack detected$scoreStr');
      notifyListeners();
      return;
    }
  ...
  double _faceCropBrightness(RawFrameData frame, Rect bbox) {
    ...
    if (Platform.isIOS) {
      ...
      switch (frame.sensorOrientation) {
        case 270:
          nl = bbox.top; nt = h - 1 - bbox.right; nr = bbox.bottom; nb = h - 1 - bbox.left;
        case 90:
          nl = w - 1 - bbox.bottom; nt = bbox.left; nr = w - 1 - bbox.top; nb = bbox.right;
        case 180:
          nl = w - 1 - bbox.right; nt = h - 1 - bbox.bottom;
          nr = w - 1 - bbox.left; nb = h - 1 - bbox.top;
        default:
          nl = bbox.left; nt = bbox.top; nr = bbox.right; nb = bbox.bottom;
      }
      ...
      for (int y = y0; y < y1; y += 8) {
        for (int x = x0; x < x1; x += 8) {
          final idx = y * w + x;
          if (idx < w * h) { sum += bytes[idx] & 0xFF; count++; }
        }
      }
    }
  ...
  double _faceCropBrightnessVariance() {
    ...
    final variance = hist.map((v) => (v - mean) * (v - mean)).reduce((a, b) => a + b) / hist.length;
    return variance;
  }
  ...
  double _faceCropLaplacianVariance(RawFrameData frame, Rect bbox) {
    ...
    if (Platform.isIOS) {
      ...
      switch (frame.sensorOrientation) {
        case 270:
          nl = bbox.top;             nt = h - 1 - bbox.right;
          nr = bbox.bottom;          nb = h - 1 - bbox.left;
        case 90:
          nl = w - 1 - bbox.bottom;  nt = bbox.left;
          nr = w - 1 - bbox.top;     nb = bbox.right;
        case 180:
          nl = w - 1 - bbox.right;   nt = h - 1 - bbox.bottom;
          nr = w - 1 - bbox.left;    nb = h - 1 - bbox.top;
        default:
  ...
  void _updateRegionBrightness(RawFrameData frame, Rect bbox) {
    ...
    for (int gy = 0; gy < _hetGridSize; gy++) {
      for (int gx = 0; gx < _hetGridSize; gx++) {
        ...
        final rx0 = (bbox.left   + bw * gx       / _hetGridSize).toInt().clamp(0, iw - 1);
        final ry0 = (bbox.top    + bh * gy       / _hetGridSize).toInt().clamp(0, ih - 1);
        final rx1 = (bbox.left   + bw * (gx + 1) / _hetGridSize).toInt().clamp(1, iw);
        final ry1 = (bbox.top    + bh * (gy + 1) / _hetGridSize).toInt().clamp(1, ih);
        ...
        if (Platform.isIOS) {
          ...
          for (int y = ry0; y < ry1; y += 6) {
            for (int x = rx0; x < rx1; x += 6) {
              final i2 = y * iw + x;
              if (i2 < iw * ih) { sum += bytes[i2] & 0xFF; count++; }
            }
          }
        }
  ...
  double? _motionHeterogeneityScore() {
    ...
    for (final hist in _hetHistory) {
      final mean = hist.reduce((a, b) => a + b) / hist.length;
      final v    = hist.map((x) => (x - mean) * (x - mean))
                       .reduce((a, b) => a + b) / hist.length;
      vars.add(v);
    }
    ...
    final spatialVar = vars.map((v) => (v - meanV) * (v - meanV))
                           .reduce((a, b) => a + b) / vars.length;
  ...
  Future<void> reset() async {
    ...
    for (final h in _hetHistory) { h.clear(); }
